...
These same steps are used in both the "doc" and "doc_visualized" builds. The difference is that the
...
doc_visualized/Jamfile).

For a summary of what transient files are used in the doc_visualized build (including ones you edit to
configure a partial build), see the .gitignore file in this directory, which also includes comments
explaining each entry.

# Setup (two scripts)
The initial setup depends on the "doc" build, first to get the XML files generated by Doxygen,
...
For initial setup, after running a full "doc" build (i.e. in boost/libs/beast/doc), run the
following script:

  * ./get-last-built-doxygen-output.sh

It will blow away the doc_visualized/build directory (if it exists) and create a new one,
populated with all the XML files output by Doxygen, including index.xml. If the script fails,
you probably will need to modify it, specifically the hard-coded path of the build dir used
by Boost.Build for the regular "doc" build.

[FIXME: Don't hard-code the build directory for the "doc" build...]
...
in the doc_visualized directory (e.g. the result of a full build from earlier, at build/xml-pages.xml).
